...
<%
	...
	logger.info("kosmoEventUpdatePage.jsp 진입 >>> : ");	
	String mid = "";
	String memnum =  "";
...
$(document).ready(function(){
	...
	$(document).on('click','#update2btn', function(){
		alert("수정했습니다. >>> ");
		//리터럴객체
		$('#eventupdateForm').attr({
			"action":"sfmEventUpdate2.sfm",
			"method":"POST",
...
<% 
	Object obj = request.getAttribute("a");
		...
		if( list != null && list.size() != 0){
			SfmEventVO sevo = list.get(0); 
%>	

<div class="left">
        <h4 class="mb-3"><b>이벤트 글쓰기</b></h4>
        <br>
        <form class="validation-form" name = "eventupdateForm" id="eventupdateForm" novalidate>
        
        <div class="mb-3">
              <label for="name"><b>이벤트 번호</b></label>            
              <input type ="text" class="form-control" name="eventnum" id="eventnum" placeholder = "글번호"  value ="<%= sevo.getEventnum() %>" readonly>
         ...
         <!-- 매치제목 -->
            <div class="mb-3">
              ...
              <input type="text" class="form-control" id="eventtitle" name="eventtitle" placeholder="글제목" value ="<%= sevo.getEventtitle() %>" required>
              <div class="invalid-feedback">
                이름을 입력해주세요.
              </div>
          </div>
           ...
           <div class="mb-3">
			  ...
			  <textarea class="form-control" id="eventcontent" name="eventcontent" rows="5"><%= sevo.getEventcontent() %></textarea>
		   </div>
		...
		<div class="row">
		   ...
		   <div class="col-md-6 mb-3">
            <label for="root"><b>이벤트 마감 시간 </b></label>
          	 <input type="text" name="enddate" id="enddate" value ="<%= sevo.getEnddate() %>" class="datetimepicker form-control"/>
           </div>
        </div>  
        ...
        <div class="mb-3">
			  ...
			  <label for="formFileMultiple" class="form-label"><b>현재 이벤트 사진</b> <img src="/ttSFM/fileupload/event_photo/<%= sevo.getEventphoto() %>" border="1" width="70" height="70" alt="image" ></label>
			</div>
			
			 
          <div class="flex">
          <button class="btn btn-primary btn-lg btn-block" type="button" id="update2btn" name="update2btn">수정</button>                
			<input type="hidden" id="memnum" name="memnum" value="<%= memnum %>">
			<input type="hidden" id="memnum" name="memnum" value="<%= sevo.getEventnum() %>">
          </div>
          </div>
      </div>
      <%
